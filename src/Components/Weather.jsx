...
const Weather = () => {
  ...
  useEffect(() => {
    const fetchWeather = (lat, lon) => {
      const url = `https://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&units=metric&cnt=3&appid=${weatherApiKey}`;
      fetch(url)
        .then((response) => response.json())
        .then((data) => {
          const current = data.list[0];
          const nextDays = data.list.slice(1, 3).map((forecast) => ({
          ...
          {forecast.length > 0 && (
            <div className="col-span-1 sm:col-span-12 lg:col-span-4 grid grid-cols-2 sm:grid-cols-2 gap-2  justify-center">
              {forecast.map((day, index) => (
                ...
                >
                  <p className="text-xs sm:text-sm md:text-base font-medium">{day.day}</p>
                  <img
                    src={day.icon}
                    alt={day.description}
                    className="w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14"
                  />
                  <p className="mr-2 text-xs sm:text-sm md:text-base capitalize">{day.description}</p>
                  <p className="text-sm sm:text-base md:text-lg font-bold">{day.temperature}°C</p>
                </div>
              ))}
            </div>
          )}
